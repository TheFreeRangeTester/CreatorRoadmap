    ...
    if (error) {
      toast({
        ...
        description:
          ...
          t("creator.loadError", "Failed to load creator page"),
        variant: "destructive",
      });
      navigate("/dashboard");
    }
      ...
      <div className="bg-gradient-to-b from-blue-600 to-indigo-900 min-h-screen pb-16">
        <div className="container mx-auto px-4 max-w-3xl pt-6">
          ...
          <div ref={ideasContainerRef} className="space-y-4 max-w-xl mx-auto">
            {ideas.length === 0 ? (
              ...
              sortedIdeas.map((idea, index) => (
                ...
                >
                  <motion.div
                    className={`relative rounded-xl overflow-hidden backdrop-blur-md ${getGradientBackground(
                      index
          ...
          <div className="text-center mt-10 text-white/50 text-xs space-y-2">
            <p>
              {t(
                "creator.wantLeaderboard",
                "¿Quieres tener tu propio Leaderboard?"
              )}{" "}
              <Link
                href="/"
                className="hover:text-white transition-colors underline"
              >
                {t("creator.getStarted", "Comenzar ahora")}
              </Link>
            </p>
            <p>
              Powered by{" "}
